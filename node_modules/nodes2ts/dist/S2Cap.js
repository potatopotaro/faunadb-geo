...
var S2Cap = (function () {
    ...
    S2Cap.fromAxisAngle = function (axis, angle) {
        ...
        var d = Math.sin(angle.radians * 0.5);
     ...
     */
    S2Cap.fromAxisArea = function (axis, area) {
        // assert (S2.isUnitLength(axis));
        return new S2Cap(axis, area / (2 * S2_1.S2.M_PI));
    };
    ...
    S2Cap.prototype.area = function () {
        return Math.max(0, this.height) * S2_1.S2.M_PI * 2;
    ...
    S2Cap.prototype.angle = function () {
        ...
        return new S1Angle_1.S1Angle(Math.asin(Math.sqrt(this.height * 0.5)) * 2);
    };
    /**
    ...
    S2Cap.prototype.isValid = function () {
        return S2_1.S2.isUnitLength(this.axis) && this.height <= 2;
    };
    ...
    S2Cap.prototype.complement = function () {
        ...
        var cHeight = this.isFull() ? -1 : Math.max(this.height, 0) * -1 + 2;
    ...
    S2Cap.prototype.interiorContains = function (p) {
        ...
        return this.isFull() || S2Point_1.S2Point.sub(this.axis, p).norm2() < (this.height * 2);
    };
    /**
    ...
    S2Cap.prototype.addPoint = function (p) {
        ...
        if (this.isEmpty()) {
            ...
            var newHeight = Math.max(this.height, S2Cap.ROUND_UP * (0.5) * (dist2));
    ...
    S2Cap.prototype.addCap = function (other) {
        if (this.isEmpty()) {
            ...
            if (angle >= (S2_1.S2.M_PI)) {
                return new S2Cap(this.axis, 2); //Full cap
            }
            else {
                var d = Math.sin(angle * (0.5));
                var newHeight = Math.max(this.height, S2Cap.ROUND_UP * 2 * d * d);
                return new S2Cap(this.axis, newHeight);
            }
        }
    };
    ...
    S2Cap.prototype.getRectBound = function () {
        ...
        if (!allLongitudes) {
            ...
            var sinC = Math.cos(axisLatLng.latRadians);
            if (sinA <= (sinC)) {
                var angleA = Math.asin(sinA / (sinC));
                lng[0] = S2_1.S2.IEEEremainder(axisLatLng.lngRadians - (angleA), 2 * S2_1.S2.M_PI);
                lng[1] = S2_1.S2.IEEEremainder(axisLatLng.lngRadians + (angleA), 2 * S2_1.S2.M_PI);
            }
        }
    ...
    S2Cap.prototype.intersects = function (cell, vertices) {
        ...
        var sin2Angle = this.height * (this.height * -1 + 2); // sin^2(capAngle)
        for (var k = 0; k < 4; ++k) {
    ...
    S2Cap.prototype.contains = function (p) {
        ...
        // assert (S2.isUnitLength(p));
        return S2Point_1.S2Point.sub(this.axis, p).norm2() <= (this.height * 2);
    };
    //
